...
function editar_producto(consulta) {
  $.ajax({
    ...
    .done(function(respuesta) {
      ...
      $("#categorias_p option[text=" + contenido.nombre_categoria + "]").attr(
        "selected",
        "selected"
      );
...
function consultar_cartera(consulta) {
  $.ajax({
    ...
    .done(function(respuesta) {
      ...
      $("#nombre_cliente").html(
        contenido.nombres_cliente + " " + contenido.apellidos_cliente
      );
      ...
      consultapedidos.forEach((e, i) => {
        ...
        $("#ped").append(
          `<tr>
              ...
              <td>
           <button class="detalle" id="ver-detalle" value="${
             e.id_pedido
           }">Ver detalle</button>
              </td>
            <td>
           <button style='background-color:${color}' id="Estado_pedido" value="${
            e.id_pedido
          }">${texto}</button>
              </td>
            <td>
           <button style='background-color:red' id="Estado_pedido" value="${
             e.id_pedido
           }">Cancelar</button>
              </td>
              </tr>`
...
function perdida(idproducto, precioCompra) {
  $.ajax({
    url: uri + "/compras/perdida",
    type: "POST",
    datatype: "HTML",
    data: {
      id: idproducto,
      ...
      compra.forEach((e, i) => {
        $("#consultaCompra").append(
          `<tr>
              ...
              <td>
<button class="detalle" id="detalle-compra" value="${
            e.id_compra
          }">Ver detalle</button>
              </td>
              </tr>`
